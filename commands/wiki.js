...
wikiMessage = function(room, message) {
  if(Command.getMatch('wiki', message.body)) {
    term = Command.filterMessage('wiki', message.body);
    ...
    rest.get(url).on('complete', function(data) {
      ...
      if (description.match('may refer to\:')) {
        ...
        // if (image) {
        //   room.speak(image, function(error, response) {
        //     // console.log(response)
        //   });
        // };
      }
...
initialize = function(val) {
  val.on('TextMessage', wikiMessage);
};
